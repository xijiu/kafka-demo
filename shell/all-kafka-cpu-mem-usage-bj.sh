...
period='7d'
day_of_week_58=" and (day_of_week() >= 1 and day_of_week() <= 5 and hour() >= 0 and hour() < 10)"
...
then
    ...
    do
        ...
        do
            ...
            # 平均使用率
            cur_promql_cpu_usage_rate="avg_over_time(sum(rate(container_cpu_usage_seconds_total{namespace=\"${ns}\",pod=\"${podName}\",container=\"kafka\"}[5m]))by(namespace,container,pod)${day_of_week_58}[168h:])/max(kube_pod_container_resource_limits{resource=\"cpu\",namespace=\"${ns}\",pod=\"${podName}\",container=\"kafka\"})by(namespace,pod,container) *100"
            cpu_usage=$(prometheus_query $cur_promql_cpu_usage_rate)

            # 最大使用率
            cur_promql_cpu_max_rate="max_over_time(sum(rate(container_cpu_usage_seconds_total{namespace=\"${ns}\",pod=\"${podName}\",container=\"kafka\"}[5m]))by(namespace,container,pod)${day_of_week_58}[168h:])/max(kube_pod_container_resource_limits{resource=\"cpu\",namespace=\"${ns}\",pod=\"${podName}\",container=\"kafka\"})by(namespace,pod,container) *100"
            cpu_usage+='/'$(prometheus_query $cur_promql_cpu_max_rate)
            # P90使用率
            cur_promql_cpu_90_rate="quantile_over_time(0.9,sum(rate(container_cpu_usage_seconds_total{namespace=\"${ns}\",pod=\"${podName}\",container=\"kafka\"}[5m]))by(namespace,container,pod)${day_of_week_58}[168h:])/max(kube_pod_container_resource_limits{resource=\"cpu\",namespace=\"${ns}\",pod=\"${podName}\",container=\"kafka\"})by(namespace,pod,container) *100"
            cpu_usage+='/'$(prometheus_query $cur_promql_cpu_90_rate)
            # P99使用率
            cur_promql_cpu_99_rate="quantile_over_time(0.99,sum(rate(container_cpu_usage_seconds_total{namespace=\"${ns}\",pod=\"${podName}\",container=\"kafka\"}[5m]))by(namespace,container,pod)${day_of_week_58}[168h:])/max(kube_pod_container_resource_limits{resource=\"cpu\",namespace=\"${ns}\",pod=\"${podName}\",container=\"kafka\"})by(namespace,pod,container) *100"
            cpu_usage+='/'$(prometheus_query $cur_promql_cpu_99_rate)



            # 平均使用率
            curl_promql_mem_usage="avg_over_time(sum(container_memory_working_set_bytes{namespace=~\"${ns}\",pod=~\"${podName}\",container=~\"kafka\",container!=\"\",container!=\"POD\"})by(namespace,pod,container)${day_of_week_58}[168h:])/max(kube_pod_container_resource_limits{resource=\"memory\",namespace=~\"${ns}\",pod=~\"${podName}\",container=~\"kafka\"})by(namespace,pod,container)*100"
            ...
            curl_promql_recive_bytes_tmp="round(avg_over_time(kafka_cluster_bytes_in_one_minute_rate{instance_name=\\\"$kafkaInstanceName\\\",tenant_id=\\\"$ns\\\"}[168h])/1024/1024,0.1)"
            recive_bytes_tmp=$(cos_prometheus_query $curl_promql_recive_bytes_tmp)
            echo "abc $recive_bytes_tmp, result22222: $recive_bytes_tmp"


            curl_promql_recive_bytes="round(avg_over_time(kafka_cluster_bytes_in_one_minute_rate{instance_name=\\\"$kafkaInstanceName\\\",tenant_id=\\\"$ns\\\"} and on() (day_of_week(timestamp(kafka_cluster_bytes_in_one_minute_rate)) >= 1)[168h])/1024/1024,0.1)"
            recive_bytes=$(cos_prometheus_query $curl_promql_recive_bytes)
            echo "abc : $curl_promql_recive_bytes, result $recive_bytes"


            curl_promql_recive_bytes_max="round(max_over_time(kafka_cluster_bytes_in_one_minute_rate{instance_name=\\\"$kafkaInstanceName\\\",tenant_id=\\\"$ns\\\"}[168h])${day_of_week_58}/1024/1024,0.1)"
            recive_bytes+='/'$(cos_prometheus_query $curl_promql_recive_bytes_max)
            curl_promql_recive_bytes_90="round(quantile_over_time(0.9,kafka_cluster_bytes_in_one_minute_rate{instance_name=\\\"$kafkaInstanceName\\\",tenant_id=\\\"$ns\\\"}[168h])${day_of_week_58}/1024/1024,0.1)"
            recive_bytes+='/'$(cos_prometheus_query $curl_promql_recive_bytes_90)
            curl_promql_recive_bytes_99="round(quantile_over_time(0.99,kafka_cluster_bytes_in_one_minute_rate{instance_name=\\\"$kafkaInstanceName\\\",tenant_id=\\\"$ns\\\"}[168h])${day_of_week_58}/1024/1024,0.1)"
            recive_bytes+='/'$(cos_prometheus_query $curl_promql_recive_bytes_99)

            # 消费速率
            curl_promql_send_bytes="round(avg_over_time(kafka_cluster_bytes_out_one_minute_rate{instance_name=\\\"$kafkaInstanceName\\\",tenant_id=\\\"$ns\\\"}${day_of_week_58}[168h])/1024/1024,0.1)"
            send_bytes=$(cos_prometheus_query $curl_promql_send_bytes)
            curl_promql_send_bytes_max="round(max_over_time(kafka_cluster_bytes_out_one_minute_rate{instance_name=\\\"$kafkaInstanceName\\\",tenant_id=\\\"$ns\\\"}${day_of_week_58}[168h])/1024/1024,0.1)"
            send_bytes+='/'$(cos_prometheus_query $curl_promql_send_bytes_max)
            curl_promql_send_bytes_90="round(quantile_over_time(0.9,kafka_cluster_bytes_out_one_minute_rate{instance_name=\\\"$kafkaInstanceName\\\",tenant_id=\\\"$ns\\\"}${day_of_week_58}[168h])/1024/1024,0.1)"
            send_bytes+='/'$(cos_prometheus_query $curl_promql_send_bytes_90)
            curl_promql_send_bytes_99="round(quantile_over_time(0.99,kafka_cluster_bytes_out_one_minute_rate{instance_name=\\\"$kafkaInstanceName\\\",tenant_id=\\\"$ns\\\"}${day_of_week_58}[168h])/1024/1024,0.1)"
