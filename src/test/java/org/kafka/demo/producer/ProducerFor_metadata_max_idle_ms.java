...
public class ProducerFor_metadata_max_idle_ms {
    ...
    private static final long metadataExpireTime = 10 * 1000;
    ...
    private static void send(KafkaProducer<byte[], byte[]> kafkaProducer, ProducerRecord<byte[], byte[]> record) throws Exception {
        while (true) {
            ...
            // 休息11秒，让元数据过期
            Thread.sleep(metadataExpireTime + 5000);
        }
    }
